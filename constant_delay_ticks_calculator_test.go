...
func TestConstantDelayTicksCalculator_Next(t *testing.T) {
	t.Run("delay is always the same", func(t *testing.T) {
		ticksCalculator := MustConstantDelayTicksCalculator(500*time.Millisecond, 1*time.Hour)
		expected := []Tick{
	...
	t.Run("stop when timed out", func(t *testing.T) {
		ticksCalculator := MustConstantDelayTicksCalculator(500*time.Millisecond, 1*time.Nanosecond)

		require.Equal(t, Tick{Stop: true}, ticksCalculator.Next())
	})
	t.Run("panics for 0 config", func(t *testing.T) {
		require.Panics(t, func() {
			MustConstantDelayTicksCalculator(0, time.Second)
		})
		require.Panics(t, func() {
			MustConstantDelayTicksCalculator(time.Hour, 0)
		})
	})
}
